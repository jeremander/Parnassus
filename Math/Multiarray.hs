...
(^:^) m []           = m
(^:^) m [Nothing] = m
(^:^) (MArray {sizes = [_], entries}) [Just indices] = MArray {sizes = [length indices], cumProds = [1], entries = V.fromList [entries V.! i | i <- indices]}
