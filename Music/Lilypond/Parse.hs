...
newtype Identifier = Identifier String -- \foo
    deriving (Eq, Show)

validIdentifiers = ['a'..'z'] ++ ['A'..'Z'] ++ ['.', '-']

instance Pretty Identifier where
    pretty (Identifier s) = "\\" <> P.string s

instance HasParser Identifier where
    parser = Identifier <$> ((char '\\') *> (many1 $ oneOf validIdentifiers))
...
parseTime :: Parser TimeSig
parseTime = (,) <$> (string "\\time" *> sorc *> parseLitInt) <*> (sorc *> char '/' *> parseBaseDur)

-- | Parses an octave, possibly with an octave check.
parseOctave :: Parser (Octave, Maybe OctaveCheck)
parseOctave = swap <$> ((,) <$> (fmap (const OctaveCheck) <$> (optionMaybe $ char '=')) <*> parseOctave')
    where
        ...
        parseOctave' = parseUpOctave <|> parseDownOctave <|> pure 0

instance HasParser PitchClass where
    parser = do
        pc <- toUpper <$> oneOf "abcdefg"
