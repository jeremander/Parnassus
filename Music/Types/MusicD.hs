...
import qualified Data.List (nub, partition, sort, transpose)
...
primD q p
    ...
    where
        m = case p of
            Rest d   -> replicate (floor $ d / q) [RestD]
            Note d p -> if (d < q)
                            then [[]]
...
resolveTies q = reverse . (foldl' combine [])
    where
        ...
        combine ((_, Rest d1):ps) RestD   = ([], Rest (q + d1)) : ps
        combine ((_, Rest _):_) (Tied _) = error "cannot have tied note after rest"
...
instance (Ord a, ToPitch a) => ToMusicD Music a where
    toMusicD = fromMusic
    fromMusicD = toMusic
...
tiedNoteDistance n1 n2 = case (n1, n2) of
    (RestD, RestD)             -> (2, 0)
    (RestD, Untied _ _)        -> (3, 0)
    (RestD, Tied _)            -> (maxBound, maxBound)
    (Untied _ _, RestD)        -> (3, 0)
    (Untied _ p1, Untied _ p2) -> (1, abs (absPitch (toPitch p1) - absPitch (toPitch p2)))
    (Untied _ p1, Tied p2)     -> if (toPitch p1 == toPitch p2) then (0, 1) else (maxBound, maxBound)
    (Tied _, RestD)            -> (3, 0)
    (Tied p1, Untied _ p2)     -> (1, abs (absPitch (toPitch p1) - absPitch (toPitch p2)))
    (Tied p1, Tied p2)         -> if (toPitch p1 == toPitch p2) then (0, 0) else (maxBound, maxBound)

instance (Ord a, ToPitch a) => MusicT MusicD a where
    ...
    toMusic (MusicD q ctl m) = ctlMod $ chord lines'
        where
            ...
            m' = padListWithDefault maxlen RestD <$> m
    ...
    unChord (MusicD q ctl m) = [MusicD q ctl (pure <$> ln) | ln <- lines]
        where lines = transposeWithDefault RestD m
    ...
    quantize q mus@(MusicD q' ctl m)
        ...
        | otherwise = MusicD q ctl (mergeGroup <$> groups5)
            where
                ...
                chopNote RestD           = RestD : replicate (n - 1) RestD
